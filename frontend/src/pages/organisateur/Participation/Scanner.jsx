...
const QRScanner = () => {
  ...
  useEffect(() => {
    ...
    function onScanSuccess(decodedText) {
      ...
      // إيقاف السكاينر مؤقتاً فاش نلقاو نتيجة
    ...
    <main className="flex-1 p-6 md:p-10">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="relative group">
          ...
          <div className="relative rounded-[2.2rem] overflow-hidden bg-orange-500/5 border border-white/10 min-h-[350px] flex items-center justify-center backdrop-blur-sm">
            ...
            {/* الكتيبه اللي غاتبقا باينة (Request Camera) */}
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 z-0">
              ...
              <p className="text-white/20 text-[10px] mt-2 underline cursor-pointer hover:text-orange-500">
                Scan an Image File
              </p>
            </div>

            {/* الكاميرا - رديناها شفافة باش تبين داكشي اللي وراها فاش تكون طافية */}
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/30">Cryptage en temps réel.</p>
            </div>
          </div>
          <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center gap-4">
            <Zap size={20} className="text-orange-500" />
            <div>
              <h3 className="text-xs font-black text-white uppercase mb-0.5">Instantané</h3>
              <p className="text-[10px] text-white/30">Validation &lt; 500ms.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <ScannerModal 
        ...
        onClose={() => {
          ...
          window.location.reload(); // إعادة تحميل الصفحة لتشغيل الكاميرا من جديد
        }}
        result={scanResult}
      />

      {/* الـ CSS السحري اللي كيحيد اللون الكحل ويقاد الأزرار */}
      <style>{`
        ...
        #reader img { display: none !important; } /* إخفاء أيقونة الكاميرا الافتراضية */
      `}</style>
    </main>
  );
